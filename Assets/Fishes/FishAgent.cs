...
public class FishAgent : MonoBehaviour
{
    ...
    protected virtual void ChasePlayer()
    {
        ...
        {
            if (Vector2.Distance(playerLastSeenAtPos, transform.position) < 1f)
            {
                pointToMoveTo = playerLastSeenAtPos + (Vector2)transform.forward * 2f;
                agent.SetDestination(pointToMoveTo);
            }
        ...
        RotateInDirectionOfMovement(agent.velocity.normalized);
